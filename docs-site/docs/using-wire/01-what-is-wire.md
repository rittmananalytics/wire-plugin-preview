...
If you have ever started a data platform engagement from a freshly signed statement of work, you will know the feeling. The client has three source systems and a Looker licence, the sponsor wants to see dashboards within the month, and you know exactly how the work should be done: requirements first, then a model, then staging, integration and warehouse layers built to a naming convention, tests on every table, a runbook for the go-live and training for the people who will have to live with the result. The trouble is that the method lives in your head and in twenty years of half-remembered projects, so that every engagement begins with rebuilding the same scaffolding from memory. The AI coding assistants you have tried do not help as much as they should, either, because while they will write you a syntactically perfect SQL model in seconds, by the third model they have forgotten your naming convention, by the fifth they have invented a new grain and none of them has any idea that the data model was supposed to be approved before any models were written at all.

The problem, in other words, is not the SQL. It is the method, and the discipline of following it in the right order with the right approvals while a client is waiting. Wouldn't it be great if your assistant already knew how a data platform engagement is run, which documents come first, what has to be approved before the next step can start and what good looks like when it is finished, so that you could simply tell it what you wanted done, make the decisions that are yours to make and leave the rest to it?

That is what Wire is for. Wire is the name Rittman Analytics gives to its analytics engineering assistant, an agent that runs inside Claude Code and carries our delivery method with it, so that you direct the work in plain English while it plans the steps, produces the documents, models and dashboards, checks its own output against the method and stops whenever a decision is yours to make. Everything it produces is a file in the git repository where your engagement lives, and everything it does is recorded there as well.

To make that concrete, consider Northwind Retail, a chain of 40 stores whose regional managers receive a store performance pack once a month, built by hand from till exports and footfall counter downloads. You open Claude Code in an empty repository, point Wire at the statement of work and say that you want store performance dashboards, that there is no warehouse access for two weeks and that it should stop at every decision. Wire reads the statement of work, proposes a "dashboards-first" release together with its reason, confirms the setup with you once and drafts the requirements. Within the hour you are approving 14 requirements and looking at a working dashboard prototype in your browser, and within four days the operations director is clicking through those same dashboards in Looker on believable sample data. When the warehouse arrives two weeks later, Wire switches the platform over to live data, runs the tests, prepares the go-live runbook and drafts a training session for the regional managers. Around six of the messages you sent in all that time were decisions you would have had to make anyway; most of the rest were "carry on".

In this chapter we will first look at what makes Wire different from a general-purpose coding assistant, then at the kinds of work you can ask it to do and how a working session with it unfolds. We will then look at what you have at the end of a release, where Wire runs and what it connects to, and we will install it so that you are ready to follow a first release from start to finish in Chapter 2. Let's start though by looking at what Wire knows that a coding assistant does not.

...
If you have ever started a data platform engagement from a freshly signed statement of work, you will know the feeling. The client has three source systems and a Looker licence, the sponsor wants to see dashboards within the month, and you know how the work should be done: requirements first, then a model, then staging, integration and warehouse layers built to a naming convention, tests on every table, a runbook for the go-live and training for the people who will live with the result. The trouble is that the method lives in your head and in twenty years of half-remembered projects, so every engagement begins by rebuilding the same scaffolding from memory. AI coding assistants do not help as much as they should: they will write a syntactically perfect SQL model in seconds, but by the third model they have forgotten your naming convention, by the fifth they have invented a new grain, and none of them knows the data model was supposed to be approved before any model was written.

The problem is not the SQL. It is the method, and the discipline of following it in the right order with the right approvals while a client is waiting. Wouldn't it be great if your assistant already knew how a data platform engagement is run, which documents come first, what needs approval before the next step and what good looks like at the end, so you could tell it what you wanted, make the decisions that are yours and leave the rest to it?

That is what Wire is for. Wire is Rittman Analytics' analytics engineering assistant, an agent that runs inside Claude Code and carries our delivery method with it. You direct the work in plain English while it plans the steps, produces the documents, models and dashboards, checks its own output against the method and stops whenever a decision is yours to make. Everything it produces is a file in the git repository where your engagement lives, and everything it does is recorded there too.

Consider Northwind Retail, a chain of 40 stores whose regional managers receive a store performance pack once a month, built by hand from till exports and footfall counter downloads. You open Claude Code in an empty repository, point Wire at the statement of work and say you want store performance dashboards, there is no warehouse access for two weeks, and it should stop at every decision. Wire reads it, proposes a "dashboards-first" release with its reason, confirms the setup with you once and drafts the requirements. Within the hour you are approving 14 requirements and looking at a working dashboard prototype in your browser, and within four days the operations director is clicking through those same dashboards in Looker on believable sample data. When the warehouse arrives two weeks later, Wire switches the platform over to live data, runs the tests, prepares the go-live runbook and drafts a training session for the regional managers. Around six of your messages in all that time were decisions you would have had to make anyway; most of the rest were "carry on".

This chapter looks at what makes Wire different from a general-purpose coding assistant, the kinds of work you can ask it to do and how a working session unfolds. It then covers what you have at the end of a release, where Wire runs and what it connects to, and ends by installing Wire so you are ready to follow a first release from start to finish in Chapter 2. Let's start with what Wire knows that a coding assistant does not.

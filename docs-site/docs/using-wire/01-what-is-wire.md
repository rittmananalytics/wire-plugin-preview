...
| "Confirm." | "Engagement set up. Requirements drafted from the statement of work: 14 requirements, all checks pass, two need clarifying with the client. Approve now, or park for the client?<br/><br/>Ran: new, requirements-generate, requirements-validate" |
| "Approve, and carry the two questions to the kickoff." | "Approved, and the two questions are noted for kickoff. Starting the dashboard designs with you now, and drafting the conceptual model in the background. The two do not depend on each other.<br/><br/>Ran: requirements-review. Running: mockups-generate, conceptual_model-generate" |

The last line of each reply names the Wire command that has just done the work, or the one about to start. You do not need those names yet, and the chapters that follow keep to plain words around them; they are there so that when you do want to know what a step is called, or a client asks you, the answer has been in front of you all along. Three messages from you, and Wire has set up the engagement, written and checked the requirements, recorded your approval together with your instruction about the kickoff and started two pieces of work at once, because it could see that neither depended on the other. [Chapter 2](./02-getting-started.md) follows this release all the way to live dashboards.

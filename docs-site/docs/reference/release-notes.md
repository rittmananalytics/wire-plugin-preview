...
**Omni community patterns folded into the framework.** Three additions sourced from Omni's community Patterns category: `bi_pairs/looker_to_omni/omni_patterns.md` gives the migration agent documented Omni idioms for the constructs the converter refuses (Mustache replacements for common Liquid uses, filtered-measure and control patterns for parameter-driven tiles, merged-results alternatives, navigation and theme recipes); `wire/skills/omni/patterns.md` adds the tool-agnostic authoring cookbook (table-calc idioms, LoD cohorting, per-dialect date spines, sessionization, multi-tenant and EAV patterns); and a new `specs/utils/omni_ai_quality.md` enablement workflow tunes the model's AI surface and gates it against 10 to 20 real user questions with a deterministic verdict, run by `training-generate` on Omni engagements and tested in `wire/tests/utils/`. `omni-target-setup` also derives a dashboard theme JSON from the client brand at setup.

**Workload model routing, stage 1.** Commands are tagged with a workload tier (`mechanical`, `templated-generation`, `judgment`, `planning`) in their spec frontmatter, and the build stamps each tier's model into the generated Claude Code command — mechanical work (help, session summaries, layout migration) runs on Haiku, planning work (`/wire:new`, `/wire:autopilot`, `/wire:adopt`, sprint planning) on the top tier, everything else on the session model as before. The tag→model map is one file, `wire/model-routing.yaml`; Tier 0 lint enforces the vocabulary and a dedicated test verifies the chain through to the shipped commands. Claude Code only — Gemini CLI has no per-command model field. Distinct from the director budget's `model_tier: economy` lane override. See [Model routing](../advanced/model-routing).

**Per-command run metrics in the execution log.** Every `execution_log.md` row now carries three more columns: `Duration`, `Tokens`, and `Cost (USD)`. Duration is measured by the command itself (`date +%s` at start and finish). Token count and estimated cost are backfilled by a new plugin `Stop` hook (`hooks/wire-metrics.sh`) that reads the measured usage from the Claude Code session transcript after each turn — the model never writes an estimated figure, so a cell is either a measured value or `n/a`. Cost is priced from a built-in per-model table; an unrecognised model gets tokens but no cost. On Gemini CLI, which has no hook mechanism, duration is still recorded and the two metric cells stay `n/a`. Logs created before this change keep their shorter rows unchanged; the header gains the new columns on the next write. Opt out with `WIRE_METRICS=false`. See [Core Concepts: The execution log](../getting-started/core-concepts#the-execution-log).
...
**Automatic validation.** Validate used to be a separate step a consultant had to remember to run between generate and review. Every `-generate` command that has a matching `-validate` command for the same artifact now runs that validate step automatically once it finishes writing the artifact, folding the PASS/FAIL result into generate's own output. A new `auto_validate` front-matter field lets a generate command opt out of the default when its validate step is expensive — real compute or live external IO, like `dbt-validate`'s `dbt run`/`dbt test` or a migration/semantic-layer validate querying a live warehouse or BI tool directly — in which case generate states plainly why and that the consultant needs to trigger validate themselves. Injected at build time into the 85 of 95 `-generate` commands with a matching `-validate` counterpart, via the same mechanism [Tracing](../advanced/tracing) already uses. Nothing changes about the gate that actually matters: `review` already requires `validate: PASS` for its own artifact, so an opted-out artifact can never reach review unvalidated — the field only decides *when* validate runs, not *whether* it's required. See [Core Concepts → Automatic validation](../getting-started/core-concepts#automatic-validation).

**dbt Charts dashboards, with a design convention.** An optional `dbtcharts` artifact on `full_platform`, `dbt_development` and `dashboard_first` writes one dbt Charts dashboard per warehouse subject area as YAML in the dbt project: a deterministic scaffold inventories the folder's facts and the dimensions they join to, the agent designs each board to a shipped brief (`--auto` for every area at once, `--subject-area` for one), and `dct` validates, renders and serves them behind a generated landing page. How the boards look is a convention in the process registry, `conventions/dbtcharts.yml`, overridable per engagement at `.wire/conventions/dbtcharts.yml` like the dbt coding standards: theme, currency, KPI and trend windows, board shape, chart defaults, formats, and the naming, layout and SQL rules `lint_conventions.py --domain dbtcharts` checks. See [dbt Charts boards](#dbt-charts-boards) below and the [dbt Charts Boards](../advanced/dbt-charts) page.

**Agents Schema publication.** An optional `agents_schema` artifact on `full_platform`, `dbt_development`, `dashboard_first`, `agentic_data_stack` and `bi_migration` publishes a release's metadata into the warehouse's own `AGENTS` schema through dbt Labs' [Agents Schema](https://github.com/dbt-labs/agents_schema): the dbt manifest (models, columns, dependencies), the LookML or Omni semantic layer, and skills assembled from the release's knowledge, including a warehouse guide the consultant completes from the business rules register. A deterministic script plans the providers and writes the GitHub workflow pinned to one upstream tag, `agents.yml` and the skills; validate compares the warehouse against the plan's row counts; nothing written ever holds a credential.
...
A new optional artifact, `dbtcharts`, on `full_platform`, `dbt_development` and `dashboard_first`, after the dbt models pass validation. `/wire:dbtcharts-generate` writes one [dbt Charts](https://dbtcharts.com) dashboard per warehouse subject area: YAML in the dbt project, queries through `ref()`, validated and rendered by the `dct` CLI, served locally with `dct serve` behind a generated landing page. A deterministic script (`scripts/dbtcharts_scaffold.py`) inventories the models from the dbt manifest and catalog using Wire's dbt naming conventions, covering every fact in the subject-area folder plus the dimensions those facts join to wherever they live; the agent then designs each board to a shipped brief (data profiled first, a KPI row with prior-period deltas, a hero trend over the last 24 months, ranked breakdowns, a detail table) and records every choice. `--auto` designs every subject area at once, one lane each, without pausing; `--subject-area <folder>` designs one. `/wire:dbtcharts-validate` runs `dct validate` (structure and warehouse dry-run), requires every board to render without warnings, and lints the boards against the design convention; `/wire:dbtcharts-review` presents the renders.

**Design conventions.** How a board looks is a convention like the dbt coding standards: `conventions/dbtcharts.yml` in the process registry, overridable per engagement at `.wire/conventions/dbtcharts.yml`. It sets theme, currency, KPI and trend windows, board shape, chart defaults and formats, and carries the naming, layout and SQL rules that `lint_conventions.py --domain dbtcharts` checks. Built and tested against a live 12-subject-area BigQuery warehouse with dct 0.8, whose gotchas (build the manifest with `dbt compile`; `DATE_TRUNC(d, MONTH)` misread by the static checker; `$`-only currency presets; no static file serving) are recorded in the skill. See [dbt Charts Boards](../advanced/dbt-charts).

### Agents Schema publication

A new optional artifact, `agents_schema`, on `full_platform`, `dbt_development`, `dashboard_first`, `agentic_data_stack` and `bi_migration`. dbt Labs' [Agents Schema](https://github.com/dbt-labs/agents_schema) is a standard schema, `AGENTS`, inside Snowflake, Databricks or BigQuery, in which an agent that queries the warehouse finds governed context next to the data: `AGENTS.ROOT` lists the providers, `AGENTS.DBT_*` hold the manifest's models, columns and dependencies, `AGENTS.LOOKML_*` and `AGENTS.OMNI_*` the semantic layer, and rows keyed `skill/...` hold markdown instructions for agents. `/wire:agents_schema-generate` runs a deterministic plan (`scripts/agents_schema_plan.py`) over the release's sources and writes the GitHub workflow that publishes on every merge, one job per provider pinned to `wire/agents_schema/pinned_version.txt`, `agents.yml` for the consumer skills, and the skills directory: each `DOMAIN_REFERENCE.md` from the agentic data stack becomes `skill/<domain>` with a `uses:` derived from its folder's models, and a warehouse guide is drafted from the manifest with two `wire: complete` markers for the business definitions and caveats only a person can write. When Looker's Git integration owns the LookML in a repository of its own, `--lookml-repo owner/repo[@ref]` writes that repository a one-job workflow of its own instead of a job in the dbt repository's, since the upstream Looker workflow checks out the repository that calls it. Because the two workflow secrets usually exist already in the consultant's dbt profile, generate offers to build them from it and set them on the repository (`scripts/agents_schema_secrets.py`: one profile and target only, values never printed), or to publish straight away; `--set-secrets` answers without asking. `/wire:agents_schema-validate` runs ten checks, four static (pinned workflow, sources unchanged, skills well formed and finished, no credentials) and five against the warehouse with the plan's own `checks.sql` (one `overview` row per provider, row counts equal to the manifest's, every skill row, no stale models, other publishers listed), plus `agents.yml`. The `agentic-data-stack-developer` agent runs the commands on every release type that carries the artifact. Tested by `wire/tests/development/validate_agents_schema_plan.py`, a fixture repository round-tripped byte for byte.
...
### BI tool migration: Looker to Omni

A new release type, `bi_migration` (issue #258), for the case where the BI tool changes and the warehouse does not. First pair: `bi_pair: looker_to_omni`. Nine artifacts: a Looker audit that classifies every LookML construct as `mechanical`, `assisted` or `redesign`; a plan whose rulings (parity or redesign per dashboard, the drop list, PDT disposition, permission mapping, topic architecture, batch order) are the release director's; Omni target setup on a model branch; model batches emitted by a deterministic converter (`scripts/lookml_to_omni.py`, which also keeps a typed intermediate representation and a dependency graph); content batches written through Omni's v2 document API; a tile-level parity loop; cutover; optional enablement.

**Parity is a script.** Each compared tile has a test contract (principal, timezone, pinned as-of, explicit row limit, key fields, a typed comparator per measure). `scripts/bi_parity.py` compares the Looker and Omni result sets and returns an outcome, `PASS`, `FAIL`, `BLOCKED`, `INCONCLUSIVE`, `NOT_RUN` or `ACCEPTED_DIFFERENCE`, mapped to the verdict taxonomy. A failed source query is `BLOCKED`, never a pass; a result at the row limit is `INCONCLUSIVE`; only a named approver turns a difference into `pass_declared_deviation`. Only `pass`, `pass_qualified` and `pass_declared_deviation` satisfy the cutover gate.

**Evidence stays tied to what it measured.** The plan writes `migration/baseline.yaml`; every verdict carries an evidence fingerprint over seven components (`scripts/bi_evidence.py`). When the LookML repo or the Omni model changes, `migration-drift-generate` recomputes the fingerprints and marks the tiles whose parity is stale; a presentation-only Omni edit costs no warehouse query.

**Both client repos stay in sync.** The LookML repo is registered and snapshotted, upstream drift is classified per view and dashboard, and `omni-model-reverse-port` brings client modellers' Omni edits back with a four-way rule that never writes a conflict.

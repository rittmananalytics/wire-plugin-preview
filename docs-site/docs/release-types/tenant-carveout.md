...
- Metabase reports are first-class inventory items: region tagging adjudicates them with everything else, each wave carves its card set (`metabase-carveout`, layer decisions first, card edits last), migrates it (`metabase-migration`, manifest-gated), and proves it at the card grain (`metabase-equivalency-validate`) — the final connection cutover is gated on every card passing. Where the tenant's target is a separately-hosted Metabase deployment rather than a second connection in the shared instance, `/wire:metabase-carveout-transport` (v3.11.8, #203) moves the signed-off card and dashboard set onto it: source strictly read-only, target writes additive-only, database ids mapped through a consultant-confirmed table (never name-guessed), idempotent by recorded target id, and `metabase-equivalency-validate` compares across the two instances via the transport manifest's id map.

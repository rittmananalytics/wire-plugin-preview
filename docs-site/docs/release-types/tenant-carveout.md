...
If you run a shared multi-tenant data platform and one of your tenants needs to leave it, taking their data, their models and their access model with them into a project of their own, you face a problem on top of the ordinary platform migration: every object you move has to be separated from the other tenants' data first, and the one wrong answer, moving something unfiltered, looks exactly like a real result until another tenant's rows turn up on the wrong side of a residency boundary. The Tenant Carve-out release type is Wire's answer to that problem. It extracts a single tenant's data, models and access model from a shared multi-tenant platform into the tenant's own target project, isolated from every other tenant, and it is delivered as a scope of the [Platform Migration](./platform-migration) release type: `migration.scope: tenant_carveout` in `status.md`, set at `/wire:new`. As such, it reuses the whole platform-migration command set and adds to it five carve-out command families, per-object tenant scoping and an isolation proof before cutover.

This page documents the release type as of **v3.11.4**.

A carve-out runs in one of two shapes, and which one you are in depends on where the parent migration has got to:
...
Everything else in the flow is identical between the two shapes, so for the rest of this page we will point out the dbt step where the two differ and otherwise treat them as one.

We will start with how you set the release up and the keys in `status.md` that drive it, then walk through the command sequence phase by phase and look at the data stores the commands share and at how they interact. After that come the files and operating model that let the per-wave loop run as an agent fleet, a worked 90-day plan and the branching strategy that keeps a carve-out in step with its parent. Let's start though with setup.

## Setting up a carve-out release

Your first step is to run `/wire:new`, select **Platform Migration** and answer the migration-scope question with **Tenant carve-out**, at which point Wire captures the default tenant predicate. From then on the carve-out is driven by the following keys in `status.md`:
...
If your carve-out release was created before v3.11.3 it gains the tenant predicate registry through `/wire:upgrade`, which backfills it from the existing region tags (Step 6a).

## The command sequence, grouped by cadence

So what do you run, and in what order? The release runs in three phases: a set of commands you run once up front to capture the audit data and rule on the separation of every object, an iterative loop you repeat per wave to carve the tenant's data and models out and a set of closing commands you run once at the end. We will take each phase in turn, listing its commands first and then looking at what the phase settles and where the results are written.

### Phase 1: run once, up front

These commands build the evidence base and the rulings the loop consumes, and each has the standard generate / validate / review triad. The review gates in this phase are human decisions that nothing downstream may override, and the listing marks the human and safety gates among them.
...
- **How each object separates** is recorded in `migration/tenant_predicate_registry.csv`. `region-tagging-generate` seeds one row per classified item and the review turns those seeds into rulings, so that each row carries one of five separation mechanisms, or `unresolved` (see [the registry](#the-tenant-predicate-registry) below).
- **The wave schedule** is `migration/migration_batching.csv`, the authoritative execution order for the loop. From v3.11.8 a carve-out staged after a parent migration (`migration.parent_release` set) is partitioned by **readiness**, not by domain or build order (`partition_mode: readiness_waves`, selected automatically, overridable with `--partition-mode`): wave assignment comes from each model's rule state in the predicate registry, whether its parent-release translation is merged on the client main and which rule groups the client has approved, and each approval group is its own sub-batch that unlocks on the client's answer without re-partitioning. Shipped waves are preserved as `B00`; unresolved and exclusion-pending rows route to named `PEN-*` "holding pens"; and validate checks that no shippable wave reads a later wave or a pen. This generalises a documented in-engagement deviation that re-partitioned 1,494 models after the domain cut drifted within days (#199).
...
This is the part that runs over and over, one wave at a time, until every in-scope object is in the tenant project and verified, and it is also the part designed for fully-agentic delivery, which we come to [below](#enabling-fully-agentic-delivery). Each pass through the loop has five steps:
...
Six rules govern how the loop behaves between those steps:

- **Waves are the reporting unit, not the work unit.** Work is pull-based over the delivery stage ladder in the register (translate/relocate, validate + lint, build, equivalence, PR), and a model advances whenever its inputs allow, whatever its wave.
...
Once the loop has exited, what remains is to prove the isolation, cut over, write the migration report and archive the release:
...
The logical-access UAT is what proves the isolation deliverable: every IAM boundary in `04_security.sql` gets positive tests and at least one negative test (query another tenant's project, expect permission denied; query a shared table, expect zero other-tenant rows). A negative test that returns another tenant's data fails the gate regardless of the positives and routes back to `target-setup`. It follows that cutover is blocked until this review is approved, and since re-running the security DDL after sign-off invalidates the attestation, the order of these closing steps matters.

## Key data stores

Where does all of this get written, and who reads it back? The table below lists every data store the carve-out commands share, with all paths relative to the release folder (`.wire/releases/<release>/`). Three of them, the tenant predicate registry, the register and the verdict log, matter most and are described in their own sections after the table.
...
Why does the carve-out need a registry when `status.md` already holds a tenant predicate? Because `migration.tenant_predicate` is one string, and a real carve-out needs several separation mechanisms at once. `migration/tenant_predicate_registry.csv` (contract: `specs/utils/tenant_predicate_registry.md`) resolves separation one object at a time.
...
The registry has a lifecycle of its own. `region-tagging-generate` seeds one row per classified item (confident-region items seed as `object_carve`; shared items carrying the predicate's column seed as `row_predicate` from the default predicate; everything else seeds `unresolved`), `region-tagging-review` turns those seeds into rulings (`resolved_by: adjudication`) and `dbt-carveout-relocate-generate` writes resolutions back as its ladder resolves models, with its row-distribution probes landing as **proposals** at `confidence: medium`, with their query and result as provenance, for a reviewer to accept or reject. A row whose `resolved_by` is `adjudication` or `manual` is never overwritten by a re-run; a conflicting machine result is reported as a conflict and left as the ruling says.

Two write-side rules arrived in v3.11.8 (#200). First, every writer quotes `expression` values at the CSV-writer level (RFC 4180: fields containing commas, quotes or newlines are double-quoted), because 3.11.x writers truncated every semi-join and regex predicate at its first comma (18 of 88 wave-1 models on the live engagement) and the rows still passed schema checks. Second, every consuming validate runs a well-formedness check on non-empty expressions (balanced parentheses, closed quotes, no dangling Jinja) that **blocks** with reason `malformed_expression` rather than letting a half-rule flow into comparisons and client-facing rule packs.

The unresolved rule is mechanical and universal, because unfiltered is the one wrong answer that looks like a real result. An unfiltered source-side query returns every tenant's rows, so a comparison fails for a reason unrelated to the migration; an unfiltered bulk copy moves another tenant's data across the residency boundary, which deleting rows afterwards does not undo. As such, every consumer refuses to proceed with an unresolved object, each in its own way:
...
Phase 2 is built to run as an agent fleet: one human release director, one orchestrating session and six to 12 "flat" lane agents working the stage ladder in parallel (`specs/utils/migration_fleet.md`). That only works if a lane can die at any moment without losing work, if no two writers collide and if spend stays inside budget, and the framework creates the files below to make those three things hold:
...
Recovery is contractual, not best-effort, and rests on four rules:
...
So how do you switch the fleet on? There is no `/wire:fleet` command. The fleet operating model is invoked by the release director in conversation, once the run-once phase has produced everything the loop needs, and the orchestrating session then follows `specs/utils/migration_fleet.md` and `specs/utils/migration_agent_delegate.md`.

### Preconditions

Before starting the fleet, confirm the following six things:
...
3. **The legal and safety gates are through**: the data-residency assessment has DPO/legal sign-off, `target-setup-review` is approved with `04_security.sql` applied and the first wave's `bulk-copy-migration-review` has authorised its pilot copy.
...
The director speaks in intents and rulings, not commands: "run wave B02 as a fleet", "ship everything that's ready", "carry on and update me when the lanes finish". The orchestrating session then does five things:

1. **Dispatches lanes** as `wire:migration-specialist` subagents. Translation and relocation runs split into groups of about five models per agent (three for complex models, up to eight for simple ones), and equivalency scopes over 50 objects partition into one lane per schema, layer or domain. Every lane brief states four things: the lane's state-file path and resume contract, its declared tree ownership, its budget line and the flat-lane rule (no sub-agents below a lane). Passing `--inline` to any command opts out of delegation.
...
3. **Applies the park rule**: the three human gates (region-tagging adjudication, DPO/legal residency sign-off, isolation UAT sign-off) are "park points", not lane stalls. An item waiting on a ruling parks in the queue and its lane moves to the next runnable item, so nothing idles waiting on a human.
...
**② The lane roster.** Relocation slices of about five models each (smaller for complex models), one build lane per target project, one comparison lane per schema, the bulk-copy monitor and the evidence lane that prepares adjudication material without ruling on it.
...
A worked plan for a carve-out of realistic scale, so that you can see how the phases above spread across a calendar: **1,400 dbt models, 600 Hightouch reverse-ETL syncs, 400 Metabase reports and 60 Airflow DAGs** (2,460 inventory items), delivered in **ten equal waves of 246 items** (140 models + 60 syncs + 40 cards + 6 DAGs per wave) over 90 days. It rests on four assumptions:

- The staged shape: the parent migration has landed, so dbt models relocate (`dbt-carveout-relocate`) rather than re-translate.
- The fleet operating model runs Phase 2: waves start five days apart and take about 12 days each, so two to three waves are in flight at once. Day numbers are project days from a nominal Monday start; the chart's dates are illustrative.
...
- Metabase reports are first-class inventory items: region tagging adjudicates them with everything else, each wave carves its card set (`metabase-carveout`, layer decisions first, card edits last), migrates it (`metabase-migration`, manifest-gated) and proves it at the card grain (`metabase-equivalency-validate`), and the final connection cutover is gated on every card passing. Where the tenant's target is a separately-hosted Metabase deployment rather than a second connection in the shared instance, transport is a plan/validate/write pipeline (#203, #221): `/wire:metabase-carveout-transport-generate` produces the full rewrite plan as a dry-run artifact, the id-rewrites plus a SQL-text rewrite for every card that hardcodes a source-project `project.dataset.table` reference in its native SQL (repointing the connection alone leaves such a card silently reading shared data); `/wire:metabase-carveout-transport-validate` re-derives the scan independently and fails any unaccounted reference or guessed mapping; `/wire:metabase-carveout-transport` then executes only the validated plan (source strictly read-only, target writes additive-only, database ids mapped through a consultant-confirmed table and never name-guessed, idempotent by recorded target id), and `metabase-equivalency-validate` compares across the two instances via the transport manifest's id map. Where the tenant's target is instead a separate warehouse project reachable from the **same** Metabase instance, the same-instance sibling pipeline applies (#255): `/wire:metabase-carveout-repoint-generate` plans, per warehouse-layer card, the write mode (duplicate into a target collection, or repoint the existing card in place keeping its id and everything that references it), the database and field-filter remaps and the same class of SQL-text rewrites; `/wire:metabase-carveout-repoint-validate` re-derives the scan and fails any in-place row with no staging collection; `/wire:metabase-carveout-repoint` writes, in two halves for anything in place (stage into a restricted collection, review and prove equivalency, then promote), with the card's pre-change query recorded verbatim as a rollback baseline. The two pipelines are mutually exclusive by design: transport refuses a same-instance target, repoint refuses a cross-instance one.

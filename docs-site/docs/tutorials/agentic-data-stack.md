...
Suppose you have a dbt platform that works well, a Looker semantic layer that your analysts know their way around and a request from the business for natural language querying on top of it. The platform is not the problem; the problem is the size of the semantic layer, and what that does to an AI agent's ability to answer questions accurately. In this tutorial we trace an `agentic_data_stack` release for a boutique analytics consultancy in exactly that position, with an existing, well-built dbt platform and a semantic layer too broad for a language model to navigate unaided.
...
definitions across the semantic layer, the BI tool and ad-hoc SQL, then
classifying the conflicts between them) now lives in
`specs/utils/definition_extract.md`, which both commands call so that the algorithm
cannot drift.
...
Why do AI analytics agents get answers wrong? Accuracy failures almost always have the same root cause, which is model sprawl and conflicting metric definitions. When a warehouse has 47 models with three different implementations of the same metric ("assets under management", say, computed slightly differently in three places for three historical reasons), a language model has no reliable way to know which to use, and so it will pick one. Sometimes it picks correctly; often it does not, and the error is invisible unless someone checks the SQL by hand.

The `agentic_data_stack` release type addresses this directly, and the order in which it works is the point: it does not start with the agent configuration. Instead, it starts with an audit of which models are overlapping, which metrics conflict and which explores are actually used, and from that audit comes a canonical model design (typically 10 to 15 models replacing the previous sprawl) with single ownership, documented grain and authoritative metric definitions. Only then are knowledge skills written, the agent configured and an eval suite run.

Of all the artifacts in the release, the eval suite is the most important, because it is the one that guards accuracy over time rather than at a single point. Every domain gets a YAML question-answer file (a minimum of ten questions each) together with a CI runner script that checks accuracy against every schema change, and this matters because Anthropic documented accuracy falling from 95% to 65% within a month without active eval maintenance. The eval suite, wired into CI, is what prevents that slide. As for who does the work, the audit and design phases are auto-delegated to the `agentic-data-stack-developer` specialist agent, while review commands stay in the main session, as we will see in the walkthrough.
...
Croftmere's financial services clients are asking for natural language querying across fund performance, client sector analysis and churn metrics, and the existing platform is solid, with 47 warehouse models and a Looker semantic layer of 312 dimensions and 94 measures. The problem is precisely that breadth. A language model navigating 312 dimensions without guidance will hallucinate metric definitions, particularly in financial services, where "AUM" or "period return" can mean different things depending on which model you land in. As such, the goal is not to replace the platform but to make a focused, governed subset of it reliably queryable by a language model.
...
Before we start, here is the delivery playbook for this tutorial's scenario, so that you can see the whole shape of the release before we take each phase in turn. In a live engagement, [`/wire:playbook-generate`](../reference/commands#session-and-management-commands) generates this as a Mermaid-format delivery plan, with dependency order, team assignments and target dates tailored to the specific release.
...
The command dispatches three concurrent audit agents, one each for the dataset audit, the metric audit and the query audit, and each reads the BigQuery information schema, the dbt manifest JSON and Looker query history for the preceding 90 days. Taken together, the findings give a clear picture of the platform's governance state, as the dataset audit's report shows:
...
With the audit approved and the two metric definitions settled, the agent designs the consolidation: 47 models reducing to 12, grouped by the six primary explore patterns. The consolidation logic is explicit in the design document, in that each canonical model lists the source models it replaces, the agreed grain and the authoritative metric definitions.

The most significant consolidation is `fund_performance_fct`, which replaces three overlapping models. It adopts the time-weighted, benchmark-adjusted return definition (agreed in the audit review) and adds a `calculation_method` metadata column so that downstream users can see which regulatory standard was applied to a given row.
...
The `dbt-developer` agent generates all 12 canonical models, and each has a YAML description that includes a grain statement (precisely one sentence describing what a row represents) together with the canonical measure definitions. This is not documentation for humans alone, because the grain statements feed directly into the knowledge skill YAML files in the next phase.
...
The agent generates one YAML knowledge file per domain, written directly into the dbt project alongside the mart models, and a representative excerpt from `fund_performance.yaml` looks like this:
...
The agent produces the Claude configuration file: a system prompt, one tool definition per knowledge skill domain (six tools in total) and explicit guardrails. The guardrails are non-negotiable in a financial services context: the agent must cite the knowledge skill it used for every answer, must never access raw source tables directly and must refuse requests that require combining domains in ways not covered by the example queries without flagging the limitation.
...
The suite has 40 test queries across six domains, each with expected results and tolerance definitions, and three representative entries look like this:
...
What should we make of the two failures? Both are informative rather than systemic. The date range edge case surfaces a UK financial year convention that needs one additional example query in the knowledge skill YAML, and the currency normalisation issue reveals a schema inconsistency between two models that the audit did not flag: the canonical model design assumed USD-denominated holdings were already converted, which they are not in `client_portfolio_fct`. Both are documented with named owners and estimated remediation time before the launch gate review.

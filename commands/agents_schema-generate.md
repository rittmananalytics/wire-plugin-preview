...
The workflow in CI needs the same manifest. Two ways, and the plan records which: the repository commits `target/manifest.json` (rare in Wire projects), or the workflow runs a managed `dbt parse` with the `DBT_PROFILES_YML` secret and the profile and target the plan passes as `dbt-profile-name` and `dbt-target`. Without either the plan records `no_dbt_profile` and the review's next steps say what the client has to add.

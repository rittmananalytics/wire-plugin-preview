---
description: Auto-delegation protocol for agentic data stack generate commands — dispatch to Agentic Data Stack Developer subagent when available
---
...
Look for `agents/agentic-data-stack-developer/AGENT.md` in the Wire plugin directory. The typical paths to check:
- `.claude/plugins/wire/agents/agentic-data-stack-developer/AGENT.md`
- `agents/agentic-data-stack-developer/AGENT.md`

### Step 2: Check execution context

Skip delegation if any of the following are true:
- The agent definition file is not found
- This instance is already running as a `wire:agentic-data-stack-developer` subagent (check the system prompt or context for this indicator — if in doubt, proceed inline to avoid infinite loops)
- The `--inline` flag was passed as part of the command arguments

### Step 3: Dispatch to specialist agent

If the agent definition exists and the above skip conditions are not met, spawn a `wire:agentic-data-stack-developer` subagent using the Agent tool with:
- `subagent_type`: `wire:agentic-data-stack-developer:AGENT`

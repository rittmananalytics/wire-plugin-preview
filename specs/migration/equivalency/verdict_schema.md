...
| `verdicts[].object_type` | `model` \| `snapshot` \| `reverse_etl_sync` (`reverse-etl-equivalency-validate`) \| `metabase_card` (`metabase-equivalency-validate`, #184) \| `tile` (`bi-equivalency-validate`, wire#258: a rebuilt BI dashboard tile, `model` set to `<dashboard_source_id>/<element_id>`). The merge rules are object-type-agnostic. |
